...
async def bgm_screenshoot2(content,search_menu,browse_num):   
    ...
    image_r = None
    image_r = await browse_page.locator('#columnSubjectInHomeB').screenshot()
    image_c = None
    image_c = await browse_page.locator('.comment_box_lr').screenshot()
    ...
    return f"https://bgm.tv/{browse_type}/{browse_link[browse_num]}", base64.b64encode(image_l).decode(), base64.b64encode(image_r).decode(), base64.b64encode(image_c).decode() 
