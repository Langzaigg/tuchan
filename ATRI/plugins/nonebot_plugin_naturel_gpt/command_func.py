...
def _(option_dict, param_dict, chat:Chat, chat_presets_dict:dict, user_id:str=''):
    ...
    if resolved_model:
        ...
        if current_draw_mode == 'off':
            return {'msg': "画图功能当前已关闭，请先使用 rg draw <on|auto> 开启画图，再切换模型。"}
        # 确保对应 schema 已加载
        if not anima_generate.get_schema(resolved_model):
...
def _(option_dict, param_dict, chat:Chat, chat_presets_dict:dict, user_id:str=''):
    ...
    if not anima_generate.get_schema(target):
        ok, err = anima_generate.fetch_schema_and_knowledge_sync()
        if not ok:
            return {'msg': f"加载画图规范失败: {err}"}
...
@cmd.register(route='rg/nolimit', params=['mode'])
def _(option_dict, param_dict, chat:Chat, chat_presets_dict:dict, user_id:str=''):
    """内容限制解锁开关（每群独立，持久化）"""
    mode = param_dict.get('mode', '').strip().lower()

    if not mode:
        current = chat.get_unlock_content_limit()
        status = "已解锁" if current else "未解锁"
        default = "已解锁" if config.UNLOCK_CONTENT_LIMIT else "未解锁"
        return {'msg': (
            f"当前群内容限制解锁: {status}\n"
            f"全局默认: {default}\n"
    ...
    enabled = (mode == 'on')
    chat.set_unlock_content_limit(enabled)
    PersistentDataManager.instance.save_to_file()
    status = "已解锁" if enabled else "已锁定"
    return {'msg': f"内容限制{status} (￣▽￣)-ok!"}
...
def _create_draw_task_from_json(json_str: str, chat: Chat) -> dict:
    ...
    if not draw_args.get('cfg'):
        draw_args['cfg'] = str(mc["default_cfg"])
    ...
    try:
        ...
        loop.create_task(anima_generate._do_generate(draw_args, config, send_ctx=send_ctx, task_id=task_id, model=draw_model))
    except Exception as e:
...
@cmd.register(route='rg/stat', params=['action'])
def _(option_dict, param_dict, chat:Chat, chat_presets_dict:dict, user_id:str=''):
    """运行统计：触发回复次数、各模型 token 消耗、缓存命中率、各工具调用次数（每日刷新）"""
    from .stats import stats

    # 仅管理员可用
    if user_id and user_id not in (config.ADMIN_USERID or []):
        return {'msg': "该指令仅管理员可用 (；′⌒`)"}

    action = param_dict.get('action', '').strip().lower()
    if action == 'reset':
        return {'msg': stats.reset_today(), 'is_progress': True}

    return {'msg': stats.render_today()}
