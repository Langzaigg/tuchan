...
ext_config: dict = {
    ...
    "description": "send specified tags anime picture. (usage in response: '/#AnimePic&萝莉,白丝|黑丝#/' for images with '萝莉' AND ('白丝' OR '黑丝') tags.)",
    # 参考词，用于上下文参考使用，为空则每次都会被参考(消耗token)
    "refer_word": ["涩图", "setu",'不够色','不够涩','色图'],
...
class CustomExtension(Extension):
    async def call(self, arg_dict: dict, _: dict) -> dict:
        ...
        tags = (
            ...
            else ([x for x in tag_str.split(",") if x and '图' not in x] or None)
        )
        tag_str = ",".join(tags) if tags else ""

        url = "https://api.lolicon.app/setu/v2"
        async with AsyncClient(proxies=proxy) as cli:
            data = (await cli.get(url, params={"tag": tags, "r18": r18, "exclude_ai" : True})).json()
