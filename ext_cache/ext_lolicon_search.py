...
class CustomExtension(Extension):
    def __init__(self, custom_config):
        ...
        self.exclude_ai: bool = config.get("exclude_ai", True)
        self.provide_tags: bool = config.get("provide_tags", True)
        self.send_manually: bool = config.get("send_manually", True)
    ...
    async def call(self, arg_dict: Dict[str, str], _: dict) -> dict:
        ...
        send_manually = self.send_manually
        ai_tip_tail = (
        ...
        print(pic.urls.original)
        return {
            "text": f"[来张好兔] {user_tip} (PID: {pic.pid})",
            "image": pic.urls.original.replace('i.pixiv.re','i.pixiv.nl').replace('img-original','img-master').replace('.jpg','_master1200.jpg').replace('.png','_master1200.jpg') if send_manually else None,
            "notify": {
                "sender": "system",
                "msg": (
                    f"[来张好兔]\n"
